...
void bye(void) { // obvious enough
    INFO("Goodbye");
    exit(0);
}

int find_free_index(void) { // -1 could cause problems
...
void handle_create(int nrows, int ncols) {
    ...
    m->nrows = nrows;
    m->ncols = ncols;
    m->data = calloc(nrows * ncols, sizeof(float)); // is this freed?
...
void handle_destroy(int id) {
    ...
    free(m->data); // is freed correctly
    free(m);
    matrices[id] = NULL; // is nulled
...
void handle_print(int id) {
    ...
    for(int i=0; i<m->nrows; i++) {
        for(int j=0; j<m->ncols; j++) {
            printf("%.9g\t", *ptr++); // oh god ptr - nrows and ncols can be out of sync of allocated data?
        }
        printf("\n");
    }
}
...
void handle_get(int id, int r, int c) {
    ...
    INFO("Matrix[%d][%d] = %.9g", r, c, m->data[r * m->nrows + c]); // SHOULD BE m->ncols
}
...
void handle_set(int id, int r, int c, float v) {
    ...
    m->data[r * m->nrows + c] = v; // set arbitrary data (woo)
